...
- Re-grounded the design foundations on the TELUS/UDS (Allium) system while preserving the OG Nuera dark-neon palette (per a Claude Design handoff): Inter on the four-weight scale (300/400/500/700; hero **display 300** + brand gradient), the Allium 4px spacing + radius scale, TELUS 250/300ms motion, and the semantic-token architecture. Dark-only at this cycle — a light theme + persistent toggle were added in a later cycle (`:root[data-theme="light"]` tokens). Inter stays (the proprietary HelveticaNowTELUS face was deliberately not adopted).
