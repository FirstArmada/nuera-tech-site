/* GitHub delivery: open a PR that updates reviews.json on the default branch.
 ...
 * Mirrors cloud/pricing-sync/lib/github.js with PATH = reviews.json.
 */
...
export const PATH = 'reviews.json';

export function makeClient(token) {
  return new Octokit({ auth: token });
}

export async function getCurrentReviews(octokit) {
  try {
...
export async function openReviewsPr(octokit, { newContent, currentSha, summary, date }) {
  const branch = `reviews-sync/${date}-${Date.now().toString(36)}`;
  ...
  await octokit.repos.createOrUpdateFileContents({
    ...
    message: `chore: reviews sync ${date}`,
