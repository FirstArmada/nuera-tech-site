...
// ⚡ Bolt: Cache properties instead of executing expensive regex matchers in O(N log N) time
...
function buildModel(repairs) {
  ...
  for (const r of repairs) {
    if (!map.has(r.model)) {
      // ⚡ Bolt: precompute sorting weights to avoid expensive regex during Array.sort
      map.set(r.model, { model: r.model, brand: r.brand, repairs: [], types: new Set(), _year: deviceYear(r.model), _tier: tierWeight(r.model) });
    }
