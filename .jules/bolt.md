...
## 2024-06-10 - Single-Pass Transform Optimizaton
**Learning:** Multiple array traversals `.map().max()` and `.reduce().division()` are inefficient. We can accomplish the same aggregation during a single pass (using a `for` loop).
**Action:** Always consider combining multiple map/reduce/filter steps into a single iteration block when computing aggregate statistics.
