...
## 2024-10-27 - O(1) CORS Origin Lookups
**Learning:** Using a `Set` for origin lookups in CORS middleware instead of an `Array` provides O(1) time complexity, which is measurably faster (14-28% improvement in microbenchmarks) and scales better if more origins are added.
**Action:** When performing membership checks against a static list of strings in hot paths (like middleware), default to using a `Set` rather than an `Array`.
