...
## 2026-06-14 - [String Comparison Performance] Learning: [Calling String.prototype.localeCompare inside Array.prototype.sort() callbacks is highly inefficient as it re-evaluates locale rules on every single comparison, turning into an O(N log N) performance bottleneck.] Action: [Always pre-instantiate an Intl.Collator (e.g., const collator = new Intl.Collator(undefined, { numeric: true });) outside the sort loop and use collator.compare(a, b) instead of localeCompare for a significant performance gain.]
