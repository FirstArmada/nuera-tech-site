...
## 2026-06-08 - [RegExp Recompilation inside Render Loops] Learning: [Recompiling the same regular expressions inside string parsing loops causes noticeable performance regressions, but this can be avoided by pre-compiling RegExp references.] Action: [Look for regex inside tight loops or string parsing functions, extract them as constant references, and use memoization caches where repeated string parsing occurs.]
...
## 2026-06-11 - [RegExp and String Operations in Sort Functions]
**Learning:** Performing regex matches and substring operations (like `deviceYear` and `tierWeight`) inside sort comparators (like `chronoCompare`) multiplies their execution cost by `O(N log N)`. Because `deviceYear` relies on multiple regex tests for every model parsed, it created a noticeable performance bottleneck during initial render when the devices array was sorted.
**Action:** When properties derived from string parsing or regex are needed for sorting, always pre-compute them once during object creation (e.g., inside `buildModel`) and store them on the object to read directly within the sort comparator.
