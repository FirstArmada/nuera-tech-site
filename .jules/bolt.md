...
## 2026-06-11 - [Pre-computing Sort Weights] Learning: [Calling string parsing functions containing regular expressions during array sorting incurs O(N log N) overhead, causing unnecessary re-evaluation for the same elements multiple times.] Action: [Always pre-compute and cache derived values required for sorting before the sort operation is called to limit execution to O(N).]
